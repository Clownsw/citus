...
get_merge_query_def(Query *query, deparse_context *context)
{
	...
	/*
	 * Add the source relation USING <source>
	 */
	sourceRelation = list_length(query->rtable);
	sourceRte = rt_fetch(sourceRelation, query->rtable);
	...
	/* if it's a shard, do differently */
	if (GetRangeTblKind(sourceRte) == CITUS_RTE_SHARD)
	{
		char *fragmentSchemaName = NULL;
		char *fragmentTableName = NULL;

		ExtractRangeTblExtraData(sourceRte, NULL, &fragmentSchemaName, &fragmentTableName, NULL);

		/* use schema and table name from the remote alias */
		appendStringInfo(buf, " USING %s%s",
						 only_marker(sourceRte),
						 generate_fragment_name(fragmentSchemaName, fragmentTableName));

		if(sourceRte->eref != NULL)
			appendStringInfo(buf, " %s",
					quote_identifier(get_rtable_name(sourceRelation, context)));
	}
	else if (sourceRte->rtekind == RTE_CTE)
	{
		 appendStringInfo(buf, " USING %s", quote_identifier(sourceRte->ctename));
	}
	else
	{
		appendStringInfo(buf, " USING %s%s",
						 only_marker(sourceRte),
						 generate_relation_or_shard_name(sourceRte->relid,
										context->distrelid,
										context->shardid, NIL));

		if (sourceRte->alias != NULL)
			appendStringInfo(buf, " %s",
							 quote_identifier(get_rtable_name(sourceRelation, context)));
	}
	...
	ereport(NOTICE, (errmsg("<Generated query: %s>", buf->data)));
}
								...
								List *namespaces)
{
	...
	if (relid == distrelid && distrelid != InvalidOid)
	{
